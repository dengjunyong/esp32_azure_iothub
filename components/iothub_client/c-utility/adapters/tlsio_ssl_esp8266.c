...
#define usleep(x) vTaskDelay(((x) / 1000) / portTICK_PERIOD_MS)
#define os_delay_us usleep

struct ip_addr {
  u32_t addr;
};
typedef struct ip_addr diy_ip_addr_t;
...
int openssl_thread_LWIP_CONNECTION(TLS_IO_INSTANCE* p)
{
    ...
    else
    {
        ...
        else
        {
            ...
            do {
                ...
                int err = getaddrinfo(tls_io_instance->hostname, NULL, &hints, &res);
                ...
                LogInfo("DNS lookup succeeded. %s IP=%s", tls_io_instance->hostname, inet_ntoa(*addr));
                tls_io_instance->target_ip.addr = addr->s_addr;
                LogInfo("addr %08x", tls_io_instance->target_ip.addr);
                freeaddrinfo(res);
                ret = 0;
    ...
    else
    {
        ...
        else
        {
            ...
            while(need_sent_bytes > 0)
            {
                ...
                ret = SSL_write(tls_io_instance->ssl, ((uint8_t*)buffer)+total_write, (int)size);
